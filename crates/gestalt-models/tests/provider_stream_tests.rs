...
fn openai_normalizes_multiple_tool_calls_in_order() {
    ...
    let events = events.into_iter().collect::<Result<Vec<_>, _>>().expect("events parse");
    ...
    assert!(events.iter().any(|event| matches!(event, AgentEvent::Stop { reason: StopReason::ToolUse })));
}
...
fn anthropic_normalizes_tool_use_and_usage() {
    ...
    assert!(events.iter().any(|event| matches!(event, AgentEvent::Usage { input_tokens: 12, .. })));

...
mod tests {
    ...
    fn schemas_should_include_public_contracts() {
        ...
        let read_schema = schemas.iter().find(|s| s["name"] == "read").expect("has read schema");
        ...
        assert!(read_props["start_line"]["description"].as_str().unwrap().contains("1-indexed"));
        assert!(read_props["end_line"]["default"].is_null()); // Optional field without default value

        // 2. Assert search tool schema details
        let search_schema = schemas.iter().find(|s| s["name"] == "search").expect("has search schema");
        ...
        assert!(search_props["case_insensitive"]["description"].as_str().unwrap().contains("case-insensitive"));

        // 3. Assert find_files tool schema details
        let find_schema = schemas.iter().find(|s| s["name"] == "find_files").expect("has find_files schema");
        ...
        let write_schema = schemas.iter().find(|s| s["name"] == "write").expect("has write schema");
        ...
        let patch_schema = schemas.iter().find(|s| s["name"] == "patch").expect("has patch schema");

...
impl PolicyEngine for AssertingPolicyEngine {
    async fn evaluate(&self, request: PolicyRequest) -> PolicyDecision {
        println!(
            "=== AssertingPolicyEngine evaluating request: tool_name='{}', namespace='{:?}'",
            request.tool_name, request.namespace
        );
        if request.tool_name == "mcp:mock-server:mock_tool" {
            assert_eq!(
                request.namespace,
                ToolNamespace::Mcp("mock-server".to_string())
            );
...
async fn test_runtime_mcp_policy_check_and_execution() {
    ...
    let policy_engine = AssertingPolicyEngine {
        called: called.clone(),
    };
    ...
    let tool_name_mappings =
        gestalt_core::tool_name_mapping::ToolNameMapping::build_mapping_with_resolution(
            &tools_list,
        );

    let mapping = tool_name_mappings
        .iter()
        .find(|m| match &m.internal_id.namespace {

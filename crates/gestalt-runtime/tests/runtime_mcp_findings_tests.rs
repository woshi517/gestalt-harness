...
use gestalt_mcp::{McpLifecycleMode, McpServerConfig, McpTransportConfig, McpConnectionState};
use std::sync::Arc;
...
async fn test_findings_lazy_servers_stay_disconnected() {
    let config = build_runtime_config(vec![
        ("lazy-server", McpLifecycleMode::Lazy),
    ]);
    ...
    let state = states.iter().find(|s| s.server_id.0 == "lazy-server").unwrap();
    ...
    let states = runtime.mcp_registry.get_all_states(5);
    let state = states.iter().find(|s| s.server_id.0 == "lazy-server").unwrap();
    assert_eq!(state.connection_state, McpConnectionState::Disconnected, "Lazy server was eagerly connected during run_prompt!");

    // Call get_client which initiates connection
    let _client = runtime.mcp_registry.get_client("lazy-server").await.unwrap();
...
async fn test_findings_duplicate_tool_names_scoping() {
    ...
    runtime.mcp_registry.get_client("server-a").await.unwrap().list_tools().await.unwrap();
    runtime.mcp_registry.get_client("server-b").await.unwrap().list_tools().await.unwrap();
    ...
    let has_a = descriptors.iter().any(|d| d.id.to_string() == "mcp:server-a:mock_tool");
    let has_b = descriptors.iter().any(|d| d.id.to_string() == "mcp:server-b:mock_tool");
    assert!(has_a);
    assert!(has_b);
    ...
    let planned_has_a = planned.iter().any(|d| d.id.to_string() == "mcp:server-a:mock_tool");
    let planned_has_b = planned.iter().any(|d| d.id.to_string() == "mcp:server-b:mock_tool");
    ...
    // Add only "mcp:server-a:mock_tool" to selection list
    {
        ...
        state.selected_tools.push("mcp:server-a:mock_tool".to_string());
    }
    ...
    let planned_has_b = planned.iter().any(|d| d.id.to_string() == "mcp:server-b:mock_tool");
    assert!(planned_has_a);
    assert!(!planned_has_b, "mcp:server-b:mock_tool was exposed even though only server-a was selected!");
}

#[tokio::test]
async fn test_findings_concurrent_first_use() {
    let config = build_runtime_config(vec![
        ("concurrent-server", McpLifecycleMode::Lazy),
    ]);

    let runtime = Arc::new(AgentRuntimeBuilder::new()
        .provider(Arc::new(DummyProvider))
        .tools(Arc::new(DummyToolCatalog))
        .middleware(Arc::new(DummyContextPipeline))
        .policy(Arc::new(AllowAllPolicyEngine))
    ...
    let first = &clients[0];
    for c in &clients[1..] {
        assert!(Arc::ptr_eq(first, c), "Concurrent calls spawned different McpClient instances!");
    }
}
...
async fn test_findings_failure_reporting_in_discovery() {
    ...
    assert!(res.is_err(), "broken server connection succeeded or failed silently!");
}

#[tokio::test]
async fn test_findings_event_emission_and_list_changed() {
    let config = build_runtime_config(vec![
        ("event-server", McpLifecycleMode::Lazy),
    ]);
    ...
    let client = runtime.mcp_registry.get_client("event-server").await.unwrap();
    ...
    let tool = runtime.tools.get("mcp:event-server:mock_tool").expect("Tool not found in catalog");
    ...
    let _exec_res = tool.execute(serde_json::json!({"input": "trigger_list_changed"}), &ctx).await.unwrap();
...
async fn test_findings_risk_reduction_requires_annotations() {
    ...
    // Populate caches
    runtime.mcp_registry.get_client("server-high-no-ann").await.unwrap().list_tools().await.unwrap();
    runtime.mcp_registry.get_client("server-high-with-ann").await.unwrap().list_tools().await.unwrap();

    // Look up mock_tool from server-high-no-ann
    let tool_no_ann = runtime.tools.get("mcp:server-high-no-ann:mock_tool").unwrap();
    // Risk should be Medium
    assert_eq!(tool_no_ann.risk(&serde_json::json!({})), gestalt_core::tool::RiskLevel::Medium);

    // Look up mock_tool from server-high-with-ann
    let tool_with_ann = runtime.tools.get("mcp:server-high-with-ann:mock_tool").unwrap();
    // Risk should be Low because of the read_only annotation
    assert_eq!(tool_with_ann.risk(&serde_json::json!({})), gestalt_core::tool::RiskLevel::Low);
}

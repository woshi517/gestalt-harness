...
{"v":1,"session_id":"session-export","turn_id":1,"seq":2,"ts":"2026-06-03T12:00:01Z","event":{"type":"stop","reason":"end_turn"},"redacted":false}"#;
    ...
    fs::write(run_dir.join("cost.json"), r#"{"runs":1,"input_tokens":0,"output_tokens":0,"estimated_cost_usd":0.0,"warnings":[]}"#).unwrap();
    ...
    // 1. Export JSONL
    let jsonl_rep = export_run(&config, "20260603T120000Z-session-export", ExportFormat::Jsonl).unwrap();
    assert_eq!(jsonl_rep.format, "jsonl");
    assert_eq!(jsonl_rep.content.trim(), trace_content.trim());

    // 2. Export Markdown
    let md_rep = export_run(&config, "20260603T120000Z-session-export", ExportFormat::Markdown).unwrap();
    assert_eq!(md_rep.format, "markdown");
    assert!(md_rep.content.contains("# Run Export: 20260603T120000Z-session-export"));
    ...
    let err_rep = export_run(&config, "20260603T120000Z-session-export", ExportFormat::Sharegpt);

...
async fn test_verify_run_artifacts() {
    ...
    let clean_res = verify_run(&config, "20260603T130000Z-session-verify")
        .await
        .unwrap();
    ...
    fs::write(
        artifacts_dir.join("leaked.txt"),
        "AWS_SECRET_ACCESS_KEY=abcd1234\n",
    )
    .unwrap();
    let dirty_res = verify_run(&config, "20260603T130000Z-session-verify")
        .await
        .unwrap();

...
mod tests {
    ...
    fn create_temp_workspace() -> (PathBuf, EffectiveConfig) {
        let temp_dir =
...
"#;
    ...
    fn test_slash_command_parities_in_chat_prompt() {
        let (_, config) = create_temp_workspace();
        let mut state =
    ...
    async fn test_end_to_end_slash_handlers_integration() {
        ...
        let export_res = gestalt_cli::export::export_run(
            &config,
            "run-1",
            gestalt_cli::output::ExportFormat::Jsonl,
        );
        assert!(
            export_res.is_ok(),
            "export_run failed: {:?}",

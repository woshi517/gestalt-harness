...
use gestalt_trace::{CostReport, aggregate_costs};
...
pub fn render_cost(report: &CostReport) -> String {
    let cost = report.estimated_cost_usd.map_or_else(
        || "unknown".to_string(),
        |value| format!("${value:.6}"),
    );
    let mut output = vec![
        format!("runs={} input_tokens={} output_tokens={}", report.runs, report.input_tokens, report.output_tokens),
        format!("estimated_cost={cost}"),
    ];
    output.extend(report.warnings.iter().map(|warning| format!("warning={warning}")));
    output.join("\n")
}

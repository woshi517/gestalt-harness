use gestalt_core::{ToolCatalog, error::HarnessError};
...
pub fn list_tools(_overrides: &CliOverrides) -> Result<ToolsListReport, Box<dyn std::error::Error>> {
...
pub fn inspect_tool(
    ...
    let tool = registry.get(tool_name).ok_or_else(|| {
        HarnessError::Tool(gestalt_core::error::ToolError::NotFound(tool_name.to_string()))
    })?;

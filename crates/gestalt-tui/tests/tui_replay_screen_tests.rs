mod tests {
    ...
    use gestalt_tui::tui::screens::chat::draw_chat_screen;
    use gestalt_tui::tui::services::{LineageNode, LineageTreeModel};
    use gestalt_tui::tui::state::{TuiAppState, TuiFocus, TuiModal};
    use gestalt_tui::tui::widgets::status_bar::draw_status_bar;
    ...
    fn test_fresh_tui_shows_new_session_mode() {
        ...
        terminal.draw(|f| draw_chat_screen(f, &state)).unwrap();
    ...
    fn test_viewport_width_policy_wide() {
        ...
        let mut state = TuiAppState::new(config, "session-123".to_string(), None);
        state.chrome.sidebar_open = true;
        state.chrome.terminal_width = 100;
        state.chrome.terminal_height = 15;
        state.lineage.model = Some(LineageTreeModel {
            session_id: "session-123".to_string(),
            nodes: vec![LineageNode {
